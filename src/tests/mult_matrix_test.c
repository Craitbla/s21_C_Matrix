...
START_TEST(test_mult_matrix2) {
  ...
  for (int i = 0, number = 7; i < columns; i++) {
...
START_TEST(test_mult_matrix_fail1) {
  int rows = rand() % 100 + 1;
  int columns = rand() % 100 + 1;
...
START_TEST(test_mult_matrix_fail2) {
  matrix_t A = {0};
  matrix_t B = {0};
  matrix_t result = {0};
...
START_TEST(test_mult_matrix_fail3) {
  int rows = rand() % 100 + 1;
  int columns = rand() % 100 + 1;
...
START_TEST(test_mult_matrix_fail4) {
  int rows = rand() % 100 + 1;
  int columns = rand() % 100 + 1;
  int rows1 = rand() % 100 + 1;
  int columns1 = rand() % 100 + 1;

  if (columns == rows1) rows1 += 1;
...
START_TEST(test_mult_matrix_fail5) {
  int rows = 2147483647;
...
Suite *mult_matrix_suite(void) {
  ...
  tcase_add_loop_test(tc_main, test_mult_matrix1, 0, 100);

  tcase_add_test(tc_main, test_mult_matrix2);
  tcase_add_test(tc_main, test_mult_matrix_fail1);
  tcase_add_test(tc_main, test_mult_matrix_fail2);
  tcase_add_test(tc_main, test_mult_matrix_fail3);
  tcase_add_test(tc_main, test_mult_matrix_fail4);
  tcase_add_test(tc_main, test_mult_matrix_fail5);

  tcase_add_loop_test(tc_main, test_mult_matrix_fail4, 0, 100);
